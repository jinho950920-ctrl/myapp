...
async function run() {
  ...
  const salesFiles = files.filter(f => f.startsWith('coupang_sales_쿠팡 온하인') && f.endsWith('.xlsx'))
    .sort()
    // Keeping unique files per date
    .reduce((acc, curr) => {
       const dateMatch = curr.match(/_(\d{4}-\d{2}-\d{2})/);
       if(dateMatch) acc[dateMatch[1]] = curr;
       return acc;
    }, {} as Record<string, string>);

  const adsFiles = files.filter(f => f.startsWith('coupang_ads_쿠팡 온하인') && f.endsWith('.xlsx'))
    .sort()
    .reduce((acc, curr) => {
       const dateMatch = curr.match(/_(\d{4}-\d{2}-\d{2})/);
       if(dateMatch) acc[dateMatch[1]] = curr;
  ...
  console.log('--- 쿠팡 온하인 매출 업데이트 시작 ---');
  for (const date of Object.keys(salesFiles).sort()) {
    if (date < '2026-04-14' || date > '2026-04-19') continue;
    const file = salesFiles[date];
    const fullPath = path.join(downloadsDir, file);
    try {
      const res = await parseSalesExcel(fullPath, accountAlias, date);
      console.log(`[매출] ${date} 성공: ${res.success}, 실패: ${res.fail}`);
    } catch (e: any) {
      console.error(`[매출] ${date} 파일 에러: ${e.message}`);
    }
  }

  console.log('\\n--- 쿠팡 온하인 광고비 업데이트 시작 ---');
  for (const date of Object.keys(adsFiles).sort()) {
    if (date < '2026-04-14' || date > '2026-04-19') continue;
    const file = adsFiles[date];
    const fullPath = path.join(downloadsDir, file);
    try {
      const res = await parseAdsExcel(fullPath, accountAlias, date);
      console.log(`[광고비] ${date} 성공: ${res.success}, 실패: ${res.fail}`);
    } catch (e: any) {
      console.error(`[광고비] ${date} 파일 에러: ${e.message}`);
    }
  }
